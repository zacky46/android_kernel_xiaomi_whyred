...
#ifdef CONFIG_KERNEL_XIAOMI_TULIP
...
void msm_sensor_set_sesnor_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	switch (s_ctrl->sensordata->sensor_info->position) {
#ifndef CONFIG_KERNEL_XIAOMI_TULIP  
		case BACK_CAMERA_B:
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	uint32_t                             j = 0;
#endif
#ifdef CONFIG_KERNEL_XIAOMI_WHYRED
	uint32_t                             i = 0;
#endif
		...
		for (i=0; i<CAMERA_VENDOR_EEPROM_COUNT_MAX; i++) {
			if (s_vendor_eeprom[i].eeprom_name != NULL) {
				...
				if (strcmp(slave_info->eeprom_name, s_vendor_eeprom[i].eeprom_name) == 0) {
					if (((strcmp(slave_info->eeprom_name, "lavender_s5k5e8_ofilm_i") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM)) ||
						((strcmp(slave_info->eeprom_name, "lavender_s5k5e8_sunny_ii") == 0) && (s_vendor_eeprom[i].module_id == MID_SUNNY)) ||
						((strcmp(slave_info->eeprom_name, "lavender_ov02a10_ofilm_i") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM)) ||
						((strcmp(slave_info->eeprom_name, "lavender_ov02a10_sunny_ii") == 0) && (s_vendor_eeprom[i].module_id == MID_SUNNY))) {
		...
		for (j=0; j<CAMERA_VENDOR_EEPROM_COUNT_MAX; j++) {
			if (s_vendor_eeprom[j].eeprom_name != NULL) {
				...
				if (strcmp(slave_info->eeprom_name, s_vendor_eeprom[j].eeprom_name) == 0) {
					if (((strcmp(slave_info->eeprom_name,"tulip_s5k5e8_ofilm_i") == 0) && (s_vendor_eeprom[j].module_id == MID_OFILM)) ||
						((strcmp(slave_info->eeprom_name,"tulip_s5k5e8_qtech_ii") == 0) && (s_vendor_eeprom[j].module_id == MID_QTECH)) ||
						((strcmp(slave_info->eeprom_name,"tulip_ov02a10_ofilm_ii") == 0) && (s_vendor_eeprom[j].module_id == MID_OFILM)) ||
						((strcmp(slave_info->eeprom_name,"tulip_ov02a10_sunny_i") == 0) && (s_vendor_eeprom[j].module_id == MID_SUNNY))) {
							printk("Lc module found!probe continue!\n");
						break;
					}
				} 
		...
		for (i=0; i<CAMERA_VENDOR_EEPROM_COUNT_MAX; i++) {
			if (s_vendor_eeprom[i].eeprom_name != NULL) {
				printk(" slave_info->eeprom_name=%s, s_vendor_eeprom[%d]=%s, module_id=%d\n",
					slave_info->eeprom_name, i, s_vendor_eeprom[i].eeprom_name, s_vendor_eeprom[i].module_id);
				if (strcmp(slave_info->eeprom_name, s_vendor_eeprom[i].eeprom_name) == 0) {
					if(((strcmp(slave_info->eeprom_name,"whyred_s5k5e8_ofilm_i") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM)) ||
						((strcmp(slave_info->eeprom_name,"whyred_s5k5e8_qtech_ii") == 0) && (s_vendor_eeprom[i].module_id == MID_QTECH))) {
							printk("Lc module found!probe continue!\n");
						break;
					}
				} 
